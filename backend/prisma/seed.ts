...
const pool = new Pool({ connectionString: url, ssl: isRemote ? { rejectUnauthorized: false } : undefined });
...
`.trim();
  ...
  if (adminRole) {
    const passwordHash = await require('argon2').hash('StorePassword@123');
