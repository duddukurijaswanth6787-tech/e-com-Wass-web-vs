...
export function ProductDetailClient() {
  ...
  const handleAddToCart = async () => {
    ...
    try {
      ...
      if (typeof window !== 'undefined' && window.boutique?.cart) {
        window.boutique.cart.addItem(
          {
            id: product.id,
            title: product.name,
            price: price,
            image: product.primaryImageUrl || (product.images && product.images[0]?.url) || undefined,
                  ...
                  <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
                    ...
                    <div className="pt-1">
                      <button
                        type="button"
                        onClick={() => {
                          if (typeof window !== 'undefined' && window.boutique?.whatsapp) {
                            window.boutique.whatsapp.openChat({
                              title: product.name,
                              price: price,
                              size: selectedSize || 'Free Size',
                            });
                            ...
                            const encodedText = encodeURIComponent(
                              `Hello Vasanti Creations, I am interested in ordering: ${product.name} (Size: ${selectedSize || 'Free Size'}) - Price: Rs. ${price}. Please guide me with direct ordering.`
                            );
                            window.open(`https://wa.me/919876543210?text=${encodedText}`, '_blank');
                          }
                        }}
                        className="w-full bg-emerald-600 hover:bg-emerald-700 active:scale-98 transition-all text-white text-xs font-extrabold tracking-wider py-3 rounded-xl flex items-center justify-center gap-2 shadow-sm shadow-emerald-700/20"
                      >
