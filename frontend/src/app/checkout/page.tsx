...
function CheckoutPageContent() {
  ...
  const onPlaceOrder = async () => {
    ...
    try {
      ...
      // Forward transaction payload to Central SaaS Master SDK
      if (typeof window !== 'undefined' && window.boutique?.checkout) {
        try {
          await window.boutique.checkout.placeOrder({
            customerName: selectedAddress?.name || (user ? `${user.firstName || ''} ${user.lastName || ''}`.trim() : '') || 'Customer',
            customerPhone: selectedAddress?.phone || '',
            customerEmail: user?.email || undefined,
            shippingAddress: selectedAddress?.addressLine1 || selectedAddress?.street || 'Address',
            city: selectedAddress?.city || 'Hyderabad',
            pincode: selectedAddress?.postalCode || selectedAddress?.pincode || '500033',
            couponCode: couponCode || undefined,
            paymentMethod: paymentMethod === 'RAZORPAY' ? 'RAZORPAY' : 'COD',
          });
        } catch (saasErr) {
          console.warn('Central SaaS Order dispatch notice:', saasErr);
        }
      }
