...
function UploadModal({ folderId, onClose, onDone }: UploadModalProps) {
  ...
  const handleUpload = useCallback(async () => {
    ...
    for (const file of files) {
      try {
        let publicUrl = '';
        let storageKey = `library/images/${Date.now()}-${file.name}`;

        // Direct Central SaaS SDK S3 Upload (No local storage / S3 server credentials needed)
        if (typeof window !== 'undefined' && window.boutique?.storage) {
          const saasResult = (await window.boutique.storage.upload(file, {
            title: file.name,
            category: 'media_library',
          })) as { fileUrl?: string; url?: string; id?: string };

          publicUrl = saasResult?.fileUrl || saasResult?.url || '';
          storageKey = saasResult?.id || storageKey;
          setProgress((p) => ({ ...p, [file.name]: 100 }));
        } else {
          // Fallback to library service S3 orchestration
          const res = await libraryService.getUploadUrl(file.name, file.type);
          publicUrl = res.publicUrl;
          storageKey = res.storageKey;
          await libraryService.uploadToS3(res.uploadUrl, file, (pct) => {
        ...
        await createMedia.mutateAsync({
          ...
          folderId,
          storageKey,
          publicUrl: publicUrl || `/storage/uploads/${file.name}`,
          thumbnailUrl: publicUrl || `/storage/uploads/${file.name}`,
          mediumUrl: publicUrl || `/storage/uploads/${file.name}`,
          largeUrl: publicUrl || `/storage/uploads/${file.name}`,
          checksum,
        });
