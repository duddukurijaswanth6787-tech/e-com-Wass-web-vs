...
export function useSaasLiveProducts() {
  return useQuery({
    queryKey: ['saas-products'],
    queryFn: async () => {
      if (typeof window !== 'undefined' && window.boutique?.storage) {
        const liveItems = await window.boutique.storage.fetchMedia();
        return (liveItems || []).map((item) => ({
          id: item.id,
          name: item.title || 'Vasanti Heritage Saree',
          basePrice: item.price || 18500,
          media: [
            {
              id: item.id,
              url: item.fileUrl,
              isPrimary: true,
              mediaType: item.fileUrl?.endsWith('.mp4') ? ('VIDEO' as const) : ('IMAGE' as const),
            },
          ],
          status: 'ACTIVE',
          categoryId: item.category || 'bridal',
        }));
      }
      return [];
    },
