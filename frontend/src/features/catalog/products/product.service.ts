...
export const productService = {
  uploadImage: async (file: File | Blob, filename: string, folder = 'products'): Promise<{ url: string }> => {
    // If Central SaaS SDK is available, upload directly to cloud S3 bucket and track live storage quota
    if (typeof window !== 'undefined' && window.boutique?.storage) {
      try {
        const fileObj = file instanceof File ? file : new File([file], filename, { type: file.type || 'image/png' });
        const saasResult = (await window.boutique.storage.upload(fileObj, {
          title: filename,
          category: folder,
        })) as { fileUrl?: string; url?: string };

        const uploadedUrl = saasResult?.fileUrl || saasResult?.url;
        if (uploadedUrl) {
          return { url: uploadedUrl };
        }
      } catch (saasUploadErr) {
        console.warn('Central SaaS S3 upload failed, falling back to local storage handler:', saasUploadErr);
      }
    }
