...
export const libraryService = {
  ...
  uploadToS3: async (uploadUrl: string, file: File, onProgress?: (pct: number) => void): Promise<void> => {
    // If Central SaaS SDK is available, upload directly through window.boutique.storage
    if (typeof window !== 'undefined' && window.boutique?.storage) {
      try {
        await window.boutique.storage.upload(file, { title: file.name, category: 'media_library' });
        if (onProgress) onProgress(100);
        return;
      } catch (err) {
        console.warn('Central SaaS Media upload notice:', err);
      }
    }
